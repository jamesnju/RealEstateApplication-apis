...
const propertySchema = Joi.object({
  ...
  price: Joi.number().precision(2).min(0).default(0.0),
  pricepermonth: Joi.number().precision(2).min(0).default(0.0),
  features: Joi.array().items(Joi.number()).optional(), // Ensures it's an array of strings
...
const updatepropertySchema = Joi.object({
  ...
  saleType: Joi.string()
    .valid("Sale", "Rent", "Lease")
    .default("Sale")
    .optional(), // Replace with actual enums in `propertySaleType`
  featured: Joi.boolean().optional(),
  propertyType: Joi.number().integer().positive().optional(),
  size: Joi.string().optional(),
  distance: Joi.string().optional(),
  price: Joi.number().precision(2).min(0).default(0.0).optional(),
  pricepermonth: Joi.number().precision(2).min(0).default(0.0).optional(),
  features: Joi.array().items(Joi.number()).optional(), // Ensures it's an array of strings
//   features: Joi.string().optional(),
...
export async function updatePropertyImage(
  ...
  try {
    const { error, value } = updatePropertyImageSchema.validate(req.body, {
      abortEarly: false,
    });
    ...
    let { action, publicId, propertyId } = value;
...
export async function getProperties(
  ...
  try {
    ...
    const take = isNaN(parseInt(limit as string))
      ? 200
      : parseInt(limit as string);
    const skip = isNaN(parseInt(offset as string))
      ? 0
